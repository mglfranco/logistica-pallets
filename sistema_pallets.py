...
st.set_page_config(layout="wide", page_title="Logística Master - Gestão de Galpão")
...
if 'estoque' not in st.session_state:
    ...
    st.session_state.capacidade_padrao = 41 # Nova linha de configuração global
    carregar_dados()

def inicializar_rua(nome_rua, capacidade):
    dados = []
    posicoes_uteis = []
    # Ordem de saída P01 -> PXX (Topo para Chão, Frente para Fundo)
    for f in range(1, 15):
...
with st.sidebar:
    ...
    # OPÇÃO SOLICITADA: Ajuste de capacidade padrão para novas ruas
    st.session_state.capacidade_padrao = st.number_input("Capacidade Padrão (Novas Ruas)", 1, 41, st.session_state.capacidade_padrao)
    
    st.divider()
    st.header("📍 Seleção de Rua")
    rua_sel = st.selectbox("Selecione a Rua", lista_ruas_opcoes)
    
    # Inicializa com a capacidade padrão se a rua for nova
...
if not st.session_state.estoque.empty:
    ...
    col_m1.metric("Capacidade Total Galpão", f"{total_pallets_global} Pallets")
    col_m2.metric("Ocupação Geral", f"{ocupados_global} Unid.")
    with col_m3:
        st.write(f"**Ocupação: {percentual:.1f}%**")
        st.progress(percentual / 100)
    st.divider()

# --- OPERAÇÕES (TABS) ---
tab1, tab2, tab3 = st.tabs(["📥 Entrada", "🟠 Reserva", "⚪ Saída"])
# (A lógica das abas permanece a mesma do código anterior, garantindo o funcionamento FIFO e Saída Direta)
with tab1:
...
with tab3:
    ...
    modo_saida = st.radio("Regra:", ["Somente Reservados", "Saída Direta"], horizontal=True)
